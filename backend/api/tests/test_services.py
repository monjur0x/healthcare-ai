...
def test_analyze_returns_full_report(tmp_path):
    ...
    # Disease-specific playbook rounds out the report; it is not empty.
...
def test_prepare_tabular_data_returns_features_and_labels(tmp_path):
    ...
    features, labels, scaler_params, encoder_params, imputer_params = (
        prepare_tabular_data(dataset, "Outcome", max_rows=None)
    )
    ...
    assert isinstance(imputer_params, dict)
