...
def _report_sections(report: ClinicalReport) -> list[str]:
    ...
    # Evidence retrieval is its own "agent" responsibility. The report
    # carries retrieved context under ``evidence`` regardless of the
    # explainability text in ``context``, so the without-RAG baseline
    # (no pipeline wired) yields an empty section here and completion
    # drops, restoring the RAG-vs-no-RAG discriminative signal that the
    # single-agent refactor flattened.
